...
#define rows imgs.rows
#define cols imgs.cols
...
bool isvalid(point p){
    if((p.x<0 || p.x>=rows) || (p.y<0 || p.y>cols))
        return 0;
    else if(img.at<Vec3b>(p.x,p.y)[0]<20 && img.at<Vec3b>(p.x,p.y)[1]<20 && img.at<Vec3b>(p.x,p.y)[2]<20)
        return 0;
...
Mat vis(rows,cols,CV_8UC1, Scalar(0));
    ...
    while(1)
    {
            ...
            for(int i=0;i<=rows;i++)
         {
            for(int j=0;j<=cols;j++)
            {
                if(imgs.at<Vec3b>(i,j)[0]<20&&imgs.at<Vec3b>(i,j)[1]<20&&imgs.at<Vec3b>(i,j)[2]<20)
                {}
                    else
                        img=imgs;
      ...
      for(ptr=nodes.begin(),ptr2=edges.begin();ptr!=nodes.end(),ptr2!=edges.end();ptr++,ptr2++)
        ...
        edges.push_back(v);
        line(cpy, Point {v.p1.y, v.p1.x}, Point {v.p2.y, v.p2.x}, Scalar(0, 0, 255), 1, 8);
        vis.at<uchar>(newNode.x, newNode.y) = 255;
        cpy.at<Vec3b>(newNode.x, newNode.y) = {0,0,255};
        printf("<<<<<<Iteration-%d>>>>>>>\nRandom Point generated : (%lld, %lld)\nnearestNode is (%lld, %lld)\nnewNode is (%lld, %lld)\n\n\n", i, rand.x, rand.y, nearestNode.x, nearestNode.y, newNode.x, newNode.y);
        imshow("RRTree", cpy);
        waitKey(1);
        if(cpy.at<Vec3b>(newNode.x, newNode.y)[2]>230)    break;
           if(i==4000){
    ...
    for(int i = edges.size()-1; i>=0; i--){
        if(edges[i].p2.x == tmp.x && edges[i].p2.y == tmp.y){
            ...
            qu.push_back(tmp);
            cout<<++j<<". ("<<tmp.x<<","<<tmp.y<<")\n";
            cpy.at<Vec3b>(edges[i].p2.x, edges[i].p2.y) = {0,0,255};
